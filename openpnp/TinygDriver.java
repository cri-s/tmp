...
public class TinygDriver extends AbstractSerialPortDriver implements Runnable {
    ...
    public synchronized void connect() throws Exception {
        ...
        for (int i = 0; i < 10 && !connected; i++) {
            try {
                Json response = sendCommand("{\"fv\":\"\"}", 500);
                // {"r":{"fv":0.950,"f":[1,0,10,2853]}}
                connectedVersion = response.at("r").at("fv").asDouble();
                connected = true;
                break;
            }
            catch (Exception e) {
    ...
    public void moveTo(ReferenceHeadMountable hm, Location location,
        ...
        if (sb.length() > 0) {
            ...
            synchronized (movementWaitLock) {
                Json response = sendCommand("G1 " + sb.toString());
    ...
    private int getResponseStatusCode(Json o) {
        return o.at("f").at(1).asInteger();
    }
    ...
    public Json sendCommand(String command) throws Exception {
        return sendCommand(command, -1);
    }

    public synchronized Json sendCommand(String command, long timeout)
            throws Exception {
        Json response;
        ...
        while (!disconnectRequested) {
            ...
            try {
                ...
                if (o.has("sr")) {
                    ...
                    logger.error(o.at("er").asString());
                }
                else if (o.isNull()) {
                logger.debug("Received invalid JSON syntax {NULL}");
                }
                else {
    ...
    private void processStatusReport(Json o) {
        if (o.has("stat")) {
            int stat = o.at("stat").asInteger();
            if (stat == 3||stat==1) {
                synchronized (movementWaitLock) {
